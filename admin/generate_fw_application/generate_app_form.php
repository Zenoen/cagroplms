...
if (isset($_POST['fw_id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $htmlTemplate = str_replace('{{SALUTATION}}', strtoupper($data['SALUTATION']??''), $htmlTemplate);

        $dateOnly = date('Y-m-d', strtotime($data['issuance_date'])); // Extract only the date part
        $htmlTemplate = str_replace('{{DATE_ISSUED}}', strtoupper($dateOnly), $htmlTemplate);
        $htmlTemplate = str_replace('{{DATE_EXPIRY}}', strtoupper($data['expiration_date']), $htmlTemplate);
        // Initialize TCPDF
