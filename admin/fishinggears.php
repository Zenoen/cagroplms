...
<head>
    ...
    <script src="../js/admin/edit-fishinggears.js" defer></script>
    ...
    <span id="printout" class="d-none">
        ...
        <small>
            <table class="table table-bordered " id="tableprint">
                ...
                <?php
                $sqlprint = "SELECT * FROM `fishinggears` WHERE `u_status`=4;";
                $resultprint = $conn->query($sqlprint);
                if ($resultprint->num_rows > 0) {
                    while ($row = $resultprint->fetch_assoc()) {
                        $name = $row['fg_locfname'] . ' ' . $row['fg_locmname'] . ' ' . $row['fg_loclname'] . ' ' . $row['fg_locappel'];
                        $location = $row['fg_locstreet'] . ' ' . $row['fg_locbarangay'] . ', ' . $row['fg_locmunicipal'] . ', ' . $row['fg_locprov'];
                        $gender = $row['fg_gender'];
                        $fgtype = $row['fg_type'];

                ?>
                        <tr class="border">
                            <td><?= "SPPCDDN2024-000" . $row['fg_id'] ?> </td>
                            <td><?= $gender ?> </td>
                            <td><?= $name ?> </td>
                            <td><?= $location ?></td>
                            <td><?= $fgtype ?></td>
                            <td><?= htmlspecialchars($row['fg_OR']) ?></td>
                            ...
                            <div class="abc" style="padding-top: 1rem;">
                                ...
                                <thead class="headert bg-primary text-white">
                                    ...
                                    <tbody class="table-con" id="dataTable">
                                        <?php
                                        if ($result_fishinggear->num_rows > 0) {
                                            $smsNotifier = new SMSNotifier($conn);
                                            $smsNotifier->sendExpiringNotifications('fishinggears', 'fg_id', 'fg_loccontact', 'Fishing Gear');
    
                                            while ($row = $result_fishinggear->fetch_assoc()) {
                                                $fgid = $row["fg_id"];
                                                $status = $row['u_status'];
                                                ...
                                                echo '<td><!-- Add your actions here -->
                                                    ...
                                                    <button hidden class="p-0   ps-1 pe-1 btn btn-success fw-bold generate-pdf-btn" data-id="' . htmlspecialchars($fgid) . '"><i class="bi bi-file-earmark-arrow-down"></i></button>
                                                    </div>
                        ...
                        <form id="editForm" action="functions/update.php" method="post">
                            <div>
                                <div style="display:flex;" class="space-top">
                                    ...
                                    <div style="margin-inline:10px">
                                        ...
                                        <div class="spacing">
                                            <label class="modal-label" for="gear">Reason:</label>
                                            <input class="input-xx" type="text" name="reason" id="edit-reason">
                                        </div>
                                    </div>

                                    <input type="text"  name="uemail" id="edit-email" hidden>

                                    <input type="hidden" name="form_type" value="fishinggears">
                            ...
                            <div class="d-flex justify-content-end">
                                ...
                                <!-- <div class="spacing d-none">
                                    <a class="href_nodesign" href="?page=requirements&permittype=fg_permit"><button button type="submit" name="action" class="btn" id="reqButton" value="issue_requirements">Issue Requirements</button></a>
                                </div> -->
                            ...
                            <div class="spacing m-1" id="renewButton">
                                ...
                                <button type="button"  class="btn btn-success" onclick="generateFG('generate_fg/generate.php','id')">
                ...
                <div id="viewModal" class="modal animy">
                    <div class="modal-content">
                        ...
                        <div>
                            ...
                            <!-- <a class="href_nodesign" href="?page=requirements&permittype=fg_permit"><button type="button" class="btn btn-primary">Issue
