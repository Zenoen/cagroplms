...
if (isset($_POST['saveReq'])) {
    ...
    // Get selected requirements and payment data
    $selectedRequirements = isset($_POST['requirements']) ? $_POST['requirements'] : [];
    // $paymentData = isset($_POST['payment_data']) ? json_decode($_POST['payment_data'], true) : [];
    ...
    $stmt = $conn->prepare("UPDATE `requirementfees` SET `requirements` = ?,`payment_data` = ? WHERE `permit_name` = ?");

    // Bind the parameters (s for strings)
    $stmt->bind_param("sss", $requirements,$paymentData, $permitType);
